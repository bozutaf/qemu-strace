...
{
    if (it_addr) {
        struct target_itimerval *it;

        it = lock_user(VERIFY_READ, it_addr, sizeof(*it), 1);
        if (!it) {
            print_pointer(it_addr, last);
            return;
        }
        qemu_log("{it_interval=");
        qemu_log("{tv_sec = " TARGET_ABI_FMT_ld
                 ",tv_usec = " TARGET_ABI_FMT_ld "},",
                 tswapal(it->it_interval.tv_sec),
                 tswapal(it->it_interval.tv_usec));
        qemu_log("it_value=");
        qemu_log("{tv_sec = " TARGET_ABI_FMT_ld
                 ",tv_usec = " TARGET_ABI_FMT_ld "}}%s",
                 tswapal(it->it_value.tv_sec),
                 tswapal(it->it_value.tv_usec), get_comma(last));
        unlock_user(it, it_addr, 0);
    } else {
        qemu_log("NULL%s", get_comma(last));
    }
}
